...
import { formatMessage, withModulesManager, Helmet, baseApiUrl, apiHeaders } from "@stssocialst-stp/fe-corefe-core";
import { PARAMS_ROUTE_TIPO_ENCAMINHAMENTO_FORM } from "../constants";
...
const LIST_QUERY = `query ListTiposEncaminhamento($first: Int, $offset: Int) {
  tiposEncaminhamento(first: $first, offset: $offset) {
    edges { node { id codigo nome descricao ativo } }
    totalCount
  }
}`;

const DELETE_MUTATION = `mutation DeleteTipoEncaminhamento($input: DeleteTipoEncaminhamentoMutationInput!) {
  deleteTipoEncaminhamento(input: $input) { clientMutationId internalId }
}`;

function TipoEncaminhamentoPage(props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = json?.data?.tiposEncaminhamento;
  ...
  return (
    <div className={classes.page}>
      <Helmet title={formatMessage(intl, "params", "title.tipoEncaminhamento")} />
      <Typography variant="h5" style={{ margin: "16px" }}>
        {formatMessage(intl, "params", "title.tipoEncaminhamento")}
      </Typography>
      <TableContainer component={Paper} className={classes.paper}>
        <Table>
          <TableHead className={classes.tableHeader}>
            <TableRow>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "tipoEncaminhamento.codigo")}</TableCell>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "tipoEncaminhamento.nome")}</TableCell>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "tipoEncaminhamento.descricao")}</TableCell>
              ...
              <TableRow key={row.id} hover>
                ...
                <TableCell>
                  <Tooltip title={formatMessage(intl, "params", "button.edit")}>
                    <IconButton size="small" onClick={() => history.push(`/${PARAMS_ROUTE_TIPO_ENCAMINHAMENTO_FORM}?id=${row.id}`)}>
      ...
      <Fab color="primary" className={classes.fab} onClick={() => history.push(`/${PARAMS_ROUTE_TIPO_ENCAMINHAMENTO_FORM}`)}>
        <AddIcon />
      </Fab>
    </div>
  );
...
export default withModulesManager(injectIntl(connect(mapStateToProps)(withTheme(withStyles(styles)(TipoEncaminhamentoPage)))));
