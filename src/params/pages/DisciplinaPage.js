...
import { PARAMS_ROUTE_DISCIPLINA_FORM } from "../constants";
...
const LIST_QUERY = `query ListDisciplinas($first: Int, $offset: Int) {
  disciplinas(first: $first, offset: $offset) {
    edges { node { id nome nivel ativo } }
    totalCount
  }
}`;

const DELETE_MUTATION = `mutation DeleteDisciplina($input: DeleteDisciplinaMutationInput!) {
  deleteDisciplina(input: $input) { clientMutationId internalId }
}`;

function DisciplinaPage(props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const data = json?.data?.disciplinas;
  ...
  return (
    <div className={classes.page}>
      <Helmet title={formatMessage(intl, "params", "title.disciplina")} />
      <Typography variant="h5" style={{ margin: "16px" }}>
        {formatMessage(intl, "params", "title.disciplina")}
      </Typography>
      <TableContainer component={Paper} className={classes.paper}>
        <Table>
          <TableHead className={classes.tableHeader}>
            <TableRow>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "disciplina.nome")}</TableCell>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "disciplina.nivel")}</TableCell>
          ...
          <TableBody>
            {rows.length === 0 && (
              <TableRow><TableCell colSpan={4} align="center">{formatMessage(intl, "params", "message.noData")}</TableCell></TableRow>
            )}
            {rows.map((row) => (
              <TableRow key={row.id} hover>
                ...
                <TableCell>
                  <Chip
                    size="small"
                    label={row.nivel === "BASICA" ? formatMessage(intl, "params", "disciplina.nivel.basica") : formatMessage(intl, "params", "disciplina.nivel.avancada")}
                    color={row.nivel === "BASICA" ? "primary" : "secondary"}
                  />
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Tooltip title={formatMessage(intl, "params", "button.edit")}>
                    <IconButton size="small" onClick={() => history.push(`/${PARAMS_ROUTE_DISCIPLINA_FORM}?id=${row.id}`)}>
      ...
      <Fab color="primary" className={classes.fab} onClick={() => history.push(`/${PARAMS_ROUTE_DISCIPLINA_FORM}`)}>
        <AddIcon />
      </Fab>
    </div>
  );
...
export default withModulesManager(injectIntl(connect(mapStateToProps)(withTheme(withStyles(styles)(DisciplinaPage)))));
