...
import { PARAMS_ROUTE_ESCOLA_FORM } from "../constants";
...
const LIST_QUERY = `query ListEscolas($first: Int, $offset: Int) {
  escolas(first: $first, offset: $offset) {
    edges { node { id nome codigo nivel ativo distrito { id name } localidade { id name } } }
    totalCount
  }
}`;

const DELETE_MUTATION = `mutation DeleteEscola($input: DeleteEscolaMutationInput!) {
  deleteEscola(input: $input) { clientMutationId internalId }
}`;

function EscolaPage(props) {
  ...
  return (
    <div className={classes.page}>
      <Helmet title={formatMessage(intl, "params", "title.escola")} />
      <Typography variant="h5" style={{ margin: "16px" }}>
        {formatMessage(intl, "params", "title.escola")}
      </Typography>
      <TableContainer component={Paper} className={classes.paper}>
        <Table>
          <TableHead className={classes.tableHeader}>
            <TableRow>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "escola.nome")}</TableCell>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "escola.codigo")}</TableCell>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "escola.nivel")}</TableCell>
              <TableCell className={classes.tableHeaderCell}>{formatMessage(intl, "params", "escola.distrito")}</TableCell>
          ...
          <TableBody>
            {rows.length === 0 && (
              <TableRow><TableCell colSpan={6} align="center">{formatMessage(intl, "params", "message.noData")}</TableCell></TableRow>
            )}
            {rows.map((row) => (
              <TableRow key={row.id} hover>
                ...
                <TableCell>
                  <Tooltip title={formatMessage(intl, "params", "button.edit")}>
                    <IconButton size="small" onClick={() => history.push(`/${PARAMS_ROUTE_ESCOLA_FORM}?id=${row.id}`)}>
      ...
      <Fab color="primary" className={classes.fab} onClick={() => history.push(`/${PARAMS_ROUTE_ESCOLA_FORM}`)}>
        <AddIcon />
      </Fab>
    </div>
  );
...
export default withModulesManager(injectIntl(connect(mapStateToProps)(withTheme(withStyles(styles)(EscolaPage)))));
